...
String androidPackage = 'com.wrteam.radioonline';
...
pre.AssetsAudioPlayer get assetsAudioPlayer => pre.AssetsAudioPlayer.withId('music');
final audios = <pre.Audio>[];
updateQueue({int ?ind,int ?choiceIndex,bool ?start}) async {
  ...
  print("in constant file .................$index,$choiceIndex");
 /* curPlayList!.forEach((item) => audios.add(pre.Audio.network(
        item.radio_url!,
        metas:pre. Metas(
          id: item.id!,
          title: item.name!,
  ...
  print("audio list length is ${audios.length},${curPlayList!.length}");
  await assetsAudioPlayer.open(
    pre.Playlist(audios: audios,startIndex: choiceIndex==0?0:ind!),
    showNotification: true,notificationSettings:pre.NotificationSettings(seekBarEnabled: false,),
    autoStart: start!,
  );
}
